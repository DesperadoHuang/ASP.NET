<%@ Page Language="C#" AutoEventWireup="true" CodeFile="ch10-4_1_FormView_Templet.aspx.cs" Inherits="ch10_4_1_FormView_Templet" %>
...
<body>
    <form id="form1" runat="server">
        <div>
            <asp:AccessDataSource ID="accessDataSource_1" runat="server" DataFile="~/App_Data/MyDatabase.mdb" DeleteCommand="DELETE FROM [學生成績表] WHERE [序號] = ?" InsertCommand="INSERT INTO [學生成績表] ([序號], [學號], [姓名], [性別], [資料庫成績], [系統分析成績], [程式設計成績], [計算機概論成績]) VALUES (?, ?, ?, ?, ?, ?, ?, ?)" SelectCommand="SELECT * FROM [學生成績表]" UpdateCommand="UPDATE [學生成績表] SET [學號] = ?, [姓名] = ?, [性別] = ?, [資料庫成績] = ?, [系統分析成績] = ?, [程式設計成績] = ?, [計算機概論成績] = ? WHERE [序號] = ?">
                <DeleteParameters>
            ...
            <asp:FormView ID="FormView1" runat="server" AllowPaging="True" CellPadding="4" DataKeyNames="序號" DataSourceID="accessDataSource_1" ForeColor="#333333" OnPageIndexChanging="FormView1_PageIndexChanging">
                <EditItemTemplate>
                    序號:
                ...
                <ItemTemplate>
                    ...
                    學號:<asp:Label ID="學號Label" runat="server" Text='<%# Bind("學號") %>' /><br />
                    姓名:<asp:Label ID="姓名Label" runat="server" Text='<%# Bind("姓名") %>' />
                    性別:<asp:Label ID="性別Label" runat="server" Text='<%# Bind("性別") %>' /><br />
                    <table>
                        <tr>
                            <td bgcolor="Yellow" colspan="2" style="text-align: center">成績單</td>
                        </tr>
                        <tr>
                            <td>資料庫成績:<asp:Label ID="資料庫成績Label" runat="server" Text='<%# Bind("資料庫成績") %>' /></td>
                            <td>&nbsp;</td>
                        </tr>
                        <tr>
                            <td>系統分析成績:<asp:Label ID="系統分析成績Label" runat="server" Text='<%# Bind("系統分析成績") %>' /></td>
                            <td>&nbsp;</td>
                        </tr>
                        <tr>
                            <td>程式設計成績:<asp:Label ID="程式設計成績Label" runat="server" Text='<%# Bind("程式設計成績") %>' /></td>
                            <td>&nbsp;</td>
                        </tr>
                        <tr>
                            <td>計算機概論成績:<asp:Label ID="計算機概論成績Label" runat="server" Text='<%# Bind("計算機概論成績") %>' /></td>
                            <td c>&nbsp;</td>
                        </tr>
                    </table>
